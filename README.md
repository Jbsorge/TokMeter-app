...
> If you want something more comprehensive — 15+ providers, browser cookie auth, WidgetKit widgets — check out [CodexBar](https://github.com/steipete/CodexBar) by Peter Steinberger. *TokMeter* is the simpler alternative: two providers, API keys, done.
...
Keys are saved locally to `~/.usage-monitor/config.json` so you only enter them once.
